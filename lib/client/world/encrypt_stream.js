...
class EncryptWorldStream extends Transform {
  ...
  _transform (packet, _, callback) {
    ...
    let isSessionPacket = false

    // It seems like the session packet always contains numbers
    let str = packet.toString().replace(/\s/g, '')
    ...
    let sessionNumber = (((this.session >> 6) & 0xFF) & 0x80000003)
